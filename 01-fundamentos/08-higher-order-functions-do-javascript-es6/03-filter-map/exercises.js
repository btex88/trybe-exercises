...
(() => { // First try doing alone.
  const authorWith3DotsOnName = () => {
    const regEx = /^[A-Z]/;
    const result = books.map((books) => books.author.name);
    const result2 = result.filter((name) =>
      name.startsWith(`${regEx}. ${regEx}. ${regEx}.`)
    );
    console.log(result);
    console.log(result2);
  };
...
(() => { // Evolution of the code with Miyazaki and after at Victor mentoring.
  const authorWith3DotsOnName = () => {
    return books
    .find((books) => books.author.name
      .split('')
      .filter((books) => books === '.')
      .length === 3).name;
  };
